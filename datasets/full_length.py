...
def get_KFold_dataset(preprocessing, path='untitled', n_splits=3):
    ...
    if not exists(dir_path):
        ...
        with tf.io.TFRecordWriter(path) as writer:
            zipper = zip(
                X_ds\
                    .map(lambda x: tf.ensure_shape(x, (None, 543, 3)))\
                    .map(preprocessing)\
                    .map(lambda x: tf.ensure_shape(x, (None, 164))), 
                         y_ds, g_ds, pid_ds, sid_ds)
    ...
    ds = ds.map(
        lambda d, l, g, pid, sid: (tf.ensure_shape(d, (None, 164)), l, g, pid, sid)
        )
